...
This document describes the Excel library exported from `@genix/ui/excel`.
...
- `helpers.ts`
  - Shared utilities (header normalization, layouts, default parsing, etc).
- `runtime.ts`
  - Host-injected WASM URL, application name, and translation.
- `types.ts`
...
- Every runtime lazily initializes and caches its configured WASM module.
- The host owns the WASM asset URL and optional translation policy.
