...
function collectLeafKeyMap<T>(runtime: ExcelRuntime, columns: ExcelImportOptions<T>['columns']) {
...
export async function parseExcelFile<T>(
  ...
  const excelize = await runtime.getExcelize();
  ...
  const { leaves, leafMap } = collectLeafKeyMap(runtime, columns);
