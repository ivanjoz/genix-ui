<script lang="ts">
  import { getUiRuntime } from '../runtime/index.js';
...
const liveBase64 = $derived(ui.images.getBase64(imageSrc?.src || ""))
...
const isInFlight = $derived(ui.images.isInFlight(imageSrc?.src || ""))
...
const makeImageSrc = (format?: string) => {
  ...
  if (srcUrl.substring(0, 8) !== "https://" && srcUrl.substring(0, 7) !== "http://") {
    srcUrl = ui.makeCdnRoute(folder as string, size ? `${srcUrl}-x${size}` :  srcUrl)
  }
...
const convertImageFile = async (
  ...
  if(useConvertAvif){
    ...
    let done = 0
    onStep?.(0, total)
    await Promise.all(resolutions.map(rs => ui.uploads.convertImage(file, rs.r, "avif").then(e => {
      rs.fn(e)
      onStep?.(++done, total)
    })))
  } else {
    onStep?.(0, 1)
    data.Content = await ui.uploads.convertImage(file, 1200, 'avif')
    onStep?.(1, 1)
  }
  return data
}
...
const confirmImage = async () => {
  ...
  try {
    const counterRoute = imageCounterConfig > 0
      ...
      : "image-id-counter"
    reserved = await ui.uploads.get({ route: counterRoute })
  ...
  const processID = ui.uploads.addProcess(processLabel, ui.translate('Converting image|Convirtiendo imagen') + '...', 1)
  ...
  const convertPromise = convertImageFile(fileToUpload, (done, total) => {
    ui.uploads.updateProcess(processID, '', ui.translate('Converting Image|Convirtiendo Imagen') + ` ${Math.max(done, 1)} / ${total}...`, 1)
  })

  const entry: UiInMemoryImage = {
  ...
  entry.upload = async (override?: Record<string, any>) => {
    ...
    ui.uploads.updateProcess(processID, '', ui.translate('Sending converted images...|Enviando imágenes convertidas...'), 1)
    try {
      await ui.uploads.post({
        data, route: saveAPI, refreshRoutes,
        onUploadProgress: e => {
          if(e.total){
            entry.progress = Math.round((e.loaded * 100) / e.total)
            ui.uploads.updateProcess(processID, '', ui.translate('Sending converted images...|Enviando imágenes convertidas...') + ` ${entry.progress}%`, 1)
          }
        },
      })
    } catch (error) {
    ...
    ui.images.entries.delete(finalName) // free memory; renderers fall through to the CDN AVIF
  }
  ...
  ui.images.entries.set(finalName, entry)
...
>
  {#if (imageSrc?.src || '').length === 0}
    <div class="relative flex flex-col items-center justify-center w-full h-full pointer-events-none">
      ...
      <div class="h5">{ui.translate("Upload Image|Subir Imagen")}</div>
    </div>
  {/if}
  ...
  {#if isInFlight}
    <div
      ...
      title={ui.translate('Processing image...|Procesando imagen...')}
    >
    ...
    >
      {#if imageSrc.base64 && !hideFormUseMessage && !hideForm}
        <textarea
          ...
          rows={3} placeholder={ui.translate("Name...|Nombre...")}
          onblur={(ev) => {
            ev.stopPropagation();
