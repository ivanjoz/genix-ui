...
const isOpen = $derived(!!ui.state.mobileDateLayer)
...
const syncLayerState = () => {
  const mobileDateLayer = ui.state.mobileDateLayer
...
const clearSelectedDate = () => {
  ...
  ui.state.mobileDateLayer?.onSelect(0)
  closeLayer()
}
...
const commitTypedDate = () => {
  ...
  if (parsedDate.isCompleted && parsedDate.autoCompletedDate && parsedDate.autoCompletedUnixDay) {
    ...
    ui.state.mobileDateLayer?.onSelect(parsedDate.autoCompletedUnixDay)
    return
  }
...
const selectDay = (unixDay: number) => {
  ...
  ui.state.mobileDateLayer?.onSelect(unixDay)
  closeLayer()
}
  ...
  <div class="p-6 pb-4">
    <div class="flex items-center">
      ...
      <input
        ...
        placeholder={ui.translate(ui.state.mobileDateLayer?.placeholder || "DD-MM-YYYY")}
        onkeydown={(ev) => {
      ...
      <div class="ml-8 flex shrink-0 items-center gap-8">
        <button
          ...
          aria-label={ui.translate("Clear date|Limpiar fecha")}
          onclick={() => {
            clearSelectedDate()
          }}
        >
        ...
        <button
          ...
          aria-label={ui.translate("Close date selector|Cerrar selector de fecha")}
          onclick={() => {
            commitTypedDate()
            closeLayer()
          }}
    ...
    <div class="_6 mt-8 p-8">
      <div class="flex items-center justify-between gap-8">
        <button
          class="_7 h-38 w-38"
          type="button"
          aria-label={ui.translate("Previous year|Año anterior")}
          onclick={() => {
            changeVisibleYear(-1)
          }}
        >
        ...
        <button
          class="_7 h-38 w-38"
          type="button"
          aria-label={ui.translate("Next year|Año siguiente")}
          onclick={() => {
            changeVisibleYear(1)
          }}
        >
