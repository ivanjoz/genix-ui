...
import { highlString, wordInclude, throttle } from '../utilities/ui.js';
import SvelteVirtualList from '@humanspeak/svelte-virtual-list';
import { untrack } from 'svelte';
import { Agent } from '../agent/registry';

// Local state for this modal instance
const isOpen = $derived(!!ui.state.mobileSearchLayer);
const canClearSelection = $derived(!!ui.state.mobileSearchLayer?.onClear)
...
const keyName = $derived(ui.state.mobileSearchLayer?.keyName) as string
const keyId = $derived(ui.state.mobileSearchLayer?.keyID) as string
const searchWords = $derived(searchText.toLowerCase().split(" ").filter(x => x.length > 1))

const optionsFiltered = $derived.by(() => {
  if((searchText||"").trim() === ""){
    return ui.state.mobileSearchLayer?.options || []
  } else {
    const filtered: any[] = []
    for (const opt of ui.state.mobileSearchLayer?.options || []){
...
$effect(() => {
  return Agent.register({
    ...
    select: (...ids) => {
      ...
      const matched = (ui.state.mobileSearchLayer?.options || []).find((opt: any) => String(opt[keyId]) === targetId)
      if (matched) {
        ui.state.mobileSearchLayer?.onSelect(matched)
        ui.state.mobileSearchLayer = null
      }
    },
    close: () => { ui.state.mobileSearchLayer = null },
  })
})
</script>
  ...
  <div class="flex items-center p-6">
    ...
    <div class="ml-8 flex shrink-0 items-center gap-8">
      {#if canClearSelection}
        <button class="h-40 w-40 shrink-0 _5 _5b" aria-label={ui.translate("Clear selection|Limpiar selección")}
          onclick={() => {
            ui.state.mobileSearchLayer?.onClear?.()
            ui.state.mobileSearchLayer = null
          }}
        >
      ...
      {/if}
      <button class="h-40 w-40 shrink-0 _5" aria-label={ui.translate("Close selector|Cerrar selector")}
        onclick={() => {
          ui.state.mobileSearchLayer = null
        }}
      >
          ...
          {#each optionRow as opt, columnIndex (`${rowIndex}-${columnIndex}`)}
            {#if opt}
              ...
              <div data-id="Option:{opt[keyId]}" class="_7" onclick={() => {
                ui.state.mobileSearchLayer?.onSelect(opt)
                ui.state.mobileSearchLayer = null
              }}>
                <div class="_9">
