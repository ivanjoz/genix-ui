...
import { getVTableAgentContext } from '../vTable/agentContext';
  ...
  let show = $derived(ui.state.popoverId === id);
  ...
  const buildMap = () => {
    if(ui.searchReferences.has(options)){ return }
    ...
    ui.searchReferences.set(options, { idToRecord, valueToRecord })
  }
  ...
  const clearValueIfNotFound = (target: HTMLInputElement) => {
    const valueToRecord = ui.searchReferences.get(options)?.valueToRecord || new Map()
  ...
  const handlwShowClick = () => {
    ...
    avoidBlur = true
    ui.state.popoverId = id as (number|string)
    setTimeout(() => {
      avoidBlur = false
    }, 120)
	}
  ...
  $effect(() => {
    if(saveOn && save){
      buildMap()
      untrack(() => {
        const idToRecord = ui.searchReferences.get(options)?.idToRecord || new Map()
