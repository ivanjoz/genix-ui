<script lang="ts">
	...
	const filteredMenus = $derived.by(() => {
		if (!model.length) { return [] as MenuGroup[] }

		return model
			.map((menuRecord) => {
				const allowedOptions = (menuRecord.options || []).filter(canAccess);
				return {
					...menuRecord,
	...
	function getMenuOpenFromRoute(menus: MenuGroup[], pathname: string): [number, string] {
	...
	$effect(() => {
		if (activePath) {
			menuOpen = getMenuOpenFromRoute(filteredMenus, activePath);
		}
	});
</script>
...
>
	<div class="flex items-center h-48 border-b border-gray-800/30 w-full overflow-hidden"
		class:minimal-menu={useTopMinimalMenu}
	>
