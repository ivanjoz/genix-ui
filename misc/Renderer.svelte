...
<script lang="ts">
  const { elements }: { elements: ElementAST | ElementAST[] } = $props()

  // Execute only the callback provided in the AST; the host retains action ownership.
...
{#snippet renderElement(element: ElementAST)}
  {#if element.tagName === 'BUTTON'}
    <button class={element.css} onclick={() => handleClick(element)}>
      {#if element.text}
        {element.text}
      {/if}
      {#if element.children}
