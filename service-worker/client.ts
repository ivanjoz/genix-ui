    ...
    try {
      const structuredResponse = await new Promise((resolve, reject) => {
        ...
        const requestMessage: IServiceWorkerStructuredRequest = {
          ...
          accion,
          reqID,
          enviroment: getServiceWorkerRuntime().getEnvironment(),
          content,
        }
  ...
  while (true) {
    ...
    if (value) {
      if (shouldTrackProgress) {
        getServiceWorkerRuntime().reportProgress(value.length)
      }
...
export const doInitServiceWorker = (): Promise<number> => {
  ...
  swReadyPromise = new Promise((resolve) => {
    navigator.serviceWorker.register(
      getServiceWorkerRuntime().getWorkerUrl(), { scope: '/', type: 'module' }
    ).then(() => {
...
export const sendServiceMessage = async (accion: number, content: any): Promise<any> => {
  ...
  if(accion === 3){
    ...
    if(shouldNotifyFetchLifecycle){
      getServiceWorkerRuntime().reportFetch(reqID, { url: content.route })
    }
  }
  ...
  try {
    if (content && typeof content === 'object') {
      content.__companyID__ = getServiceWorkerRuntime().getCompanyID()
    }
    ...
    let route = `${window.location.origin}/_sw_?accion=${accion}&req=${reqID}&env=${getServiceWorkerRuntime().getEnvironment()}`
    if(content.route){
    ...
    if (shouldNotifyFetchLifecycle) {
      getServiceWorkerRuntime().reportFetch(reqID, 0)
    }
  }
}
...
export const fetchCache = async (args: serviceHttpProps): Promise<FetchCacheResponse> => {
  ...
  args.routeParsed = runtime.makeRoute(args.route)
  args.__version__ = args.useCache?.ver || 1
  args.__companyID__ = runtime.getCompanyID()
  args.verifyRouteMemoryState = args.verifyRouteMemoryState ?? runtime.verifyRouteMemoryState()
